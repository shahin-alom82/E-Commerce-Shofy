...
const CartProduct = ({ product }: { product: ProductType }) => {
      ...
      return (
            <Container className="flex gap-4 border-b-2 border-gray-300 relative">
                  <div className="">
                        <Link className="" href={{ pathname: `/products/${product.id}`, query: { id: product.id } }}>
                              <Image className="h-48 w-48 border-r-2 border-gray-300 cursor-pointer" src={product?.images[0]} alt="cartImage" height={200} width={200} />
                        </Link>
                  </div>
                  <div>
                        <div>
                        ...
                        <div className="flex items-center gap-2 mt-2 text-sm">
                              ...
                              <h1 className="flex items-center gap-2"> You are saving <PriceFormate className="text-green-500 text-[14px]" amount={product?.price * (product?.discountPercentage / 100) * product?.quantity} /> upon purchase</h1>
                        </div>
                  </div>
